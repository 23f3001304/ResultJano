...
const waitForDownload = async (roll, jobId, timeoutMs = 5000) => {  // Reduced timeout from 10000 to 5000
  const jobDir = getJobDownloadDir(jobId);
  console.log(`Waiting for download of roll ${roll} in ${jobDir}, timeout: ${timeoutMs}ms`);
  const startTime = Date.now();
  
  // Check immediately first
  let files = await fs.promises.readdir(jobDir);
  if (files.length > 0) {
    console.log(`Download completed immediately for roll ${roll}`);
    return true;
  }
  
  // Setup faster polling interval
  const checkInterval = 200;  // Reduced from 500ms to 200ms for faster checks
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    if (files.length > 0) {
      ...
      console.log(`Download completed for roll ${roll} in ${elapsed}ms`);
      return true;
    }
  }
  
  console.log(`Download timeout for roll ${roll} after ${timeoutMs}ms`);
  return false;
};
  ...
  page.once('dialog', dialogHandler);
      ...
      await Promise.all([
        ...
        page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 3000 }).catch(() => {
          console.log('Navigation completed or timed out - proceeding with download check');
        })
      ]);

      // Race between dialog detection and timeout
      const dialog = await Promise.race([dialogPromise, dialogTimeoutPromise]);
      
      // Only wait for download if no dialog appeared
      let success = false;
      if (!dialog) {
